...
const kindMeta: Record<LogKind, { icon: typeof Search; color: string; ring: string }> = {
  checking: { icon: Search, color: 'text-slate-300', ring: 'ring-slate-500/30 bg-slate-500/10' },
  found: { icon: UserPlus, color: 'text-emerald-300', ring: 'ring-emerald-500/30 bg-emerald-500/10' },
  skip: { icon: SkipForward, color: 'text-amber-300', ring: 'ring-amber-500/30 bg-amber-500/10' },
  done: { icon: CheckCircle2, color: 'text-sky-300', ring: 'ring-sky-500/30 bg-sky-500/10' },
  reset: { icon: RotateCcw, color: 'text-rose-300', ring: 'ring-rose-500/30 bg-rose-500/10' },
}

/** Scrolling, timestamped story of every step the search takes. */
  ...
  return (
    <Card className="flex h-full flex-col px-4 py-3">
      <div className="mb-2 flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-slate-100">Search Story</p>
