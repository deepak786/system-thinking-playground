...
const kindMeta: Record<LogKind, { icon: typeof Ban; color: string; ring: string }> = {
  allowed: { icon: CheckCircle2, color: 'text-emerald-300', ring: 'ring-emerald-500/30 bg-emerald-500/10' },
  blocked: { icon: Ban, color: 'text-rose-300', ring: 'ring-rose-500/30 bg-rose-500/10' },
  window: { icon: TimerReset, color: 'text-amber-300', ring: 'ring-amber-500/30 bg-amber-500/10' },
  rule: { icon: Gauge, color: 'text-sky-300', ring: 'ring-sky-500/30 bg-sky-500/10' },
  reset: { icon: RotateCcw, color: 'text-rose-300', ring: 'ring-rose-500/30 bg-rose-500/10' },
}

/** Scrolling, timestamped story of every request and rule change. */
  ...
  return (
    <Card className="flex h-full flex-col px-4 py-3">
      <div className="mb-2 flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-slate-100">Server Log</p>
