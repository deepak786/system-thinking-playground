...
const kindMeta: Record<LogKind, { icon: typeof Zap; color: string; ring: string }> = {
  hit: { icon: Zap, color: 'text-emerald-300', ring: 'ring-emerald-500/30 bg-emerald-500/10' },
  miss: { icon: Turtle, color: 'text-amber-300', ring: 'ring-amber-500/30 bg-amber-500/10' },
  insert: { icon: PackagePlus, color: 'text-sky-300', ring: 'ring-sky-500/30 bg-sky-500/10' },
  evict: { icon: Trash2, color: 'text-rose-300', ring: 'ring-rose-500/30 bg-rose-500/10' },
  rule: { icon: Gauge, color: 'text-violet-300', ring: 'ring-violet-500/30 bg-violet-500/10' },
  reset: { icon: RotateCcw, color: 'text-rose-300', ring: 'ring-rose-500/30 bg-rose-500/10' },
}

/** Scrolling, timestamped story of every hit, miss, insert, and eviction. */
  ...
  return (
    <Card className="flex h-full flex-col px-4 py-3">
      <div className="mb-2 flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-slate-100">Cache Story</p>
